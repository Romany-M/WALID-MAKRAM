    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <div className={`flex justify-between items-center px-4 md:px-8 py-4 md:py-5 flex-shrink-0 border-b
        ...
        <button onClick={onClose}
          className={`flex items-center gap-2 md:gap-3 text-xs md:text-sm tracking-[0.4em] uppercase transition-colors duration-200 group
            ${isDark ? "text-white/60 hover:text-white" : "text-neutral-500 hover:text-neutral-900"}`}>
          {t.close[lang]}
          <span className={`w-8 h-8 md:w-9 md:h-9 rounded-full border flex items-center justify-center text-base md:text-lg text-[#b8955a] transition-all duration-300 group-hover:border-[#b8955a]
      ...
      <div className="flex-1 flex flex-col md:flex-row items-stretch overflow-hidden"
        ...
        <div className="flex-1 flex items-center justify-center relative p-4 md:p-16 overflow-hidden min-h-0">
          ...
          <button className={`absolute right-2 md:right-4 top-1/2 -translate-y-1/2 w-10 md:w-12 h-16 md:h-20
            flex items-center justify-center text-4xl md:text-5xl transition z-10
            ${isDark ? "text-white/15 hover:text-[#b8955a]" : "text-neutral-300 hover:text-[#b8955a]"}`}
        ...
        <motion.div key={`info-${index}`}
          ...
          className={`w-full md:w-[420px] lg:w-[500px] shrink-0 flex flex-col justify-center
                     px-6 md:px-12 py-6 md:py-14 border-t md:border-t-0 md:border-l overflow-y-auto
          ...
          <h3 className={`font-extralight leading-relaxed mb-6 md:mb-10
            ${isDark ? "text-white" : "text-neutral-900"}
            ${isAR ? "ar-section-title text-xl md:text-2xl" : "text-2xl md:text-3xl tracking-[0.15em]"}`}>
          ...
          <div className="flex gap-3 mt-8 md:mt-12">
            <button onClick={()=>onNav((index-1+total)%total)}
              className={`flex-1 border py-3 md:py-4 text-xs md:text-sm tracking-[0.3em] uppercase hover:border-[#b8955a] transition-all duration-300
    ...
    <motion.div initial={{ opacity:0, y:50 }} whileInView={{ opacity:1, y:0 }}
      ...
      <div className="overflow-hidden bg-neutral-100 dark:bg-neutral-900 relative aspect-[4/5]">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-400">
          <div className="w-10 h-10 md:w-11 md:h-11 rounded-full border border-white/60 flex items-center justify-center backdrop-blur-sm bg-black/20">
            <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 md:w-4 md:h-4 stroke-white fill-none" strokeWidth="1.6"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>
          </div>
        </div>
        <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4 bg-black/60 translate-y-full group-hover:translate-y-0 transition-transform duration-500"
             dir={isAR?"rtl":"ltr"}>
          <p className="text-white text-[9px] md:text-[10px] tracking-[0.35em] uppercase font-medium">{isAR?titleAR:title}</p>
      ...
      <div className="mt-3 md:mt-4 pb-3 border-b border-neutral-200 dark:border-neutral-800 flex justify-between items-baseline gap-2 md:gap-3"
        ...
        <div className={`flex-shrink-0 ${isAR?"text-left":"text-right"}`}>
          ...
          <span className="text-[8px] md:text-[9px] tracking-widest text-neutral-400 block">{year}</span>
        </div>
      </div>
    </motion.div>
  );
...
function SectionHeader({ label, title }: { label:string; title:string }) {
  return (
    <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
      viewport={{ once:true }} transition={{ duration:1 }} className="text-center mb-12 md:mb-20">
      <p className="text-[9px] tracking-[0.55em] text-neutral-400 uppercase mb-4 md:mb-6">{label}</p>
      <h2 className="font-light tracking-[0.3em] md:tracking-[0.5em] uppercase italic text-2xl md:text-3xl lg:text-4xl">{title}</h2>
      <div className="w-8 h-[1px] bg-[#b8955a] mx-auto mt-6 md:mt-8" />
    </motion.div>
  );
}
    ...
    <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
      ...
      <div className="flex flex-col gap-0">
        ...
        <button onClick={handleSend} disabled={status === "sending"}
          className={`mt-10 md:mt-14 group flex items-center justify-between w-full border-2 px-6 md:px-8 py-3.5 md:py-4
...
export default function Home() {
  ...
  useEffect(() => {
    if (loading && localStorage.getItem("preloaderShown")) {
      _preloaderShown = true; setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    const load = async () => { const data = await loadConfig(); setCfg(data); };
    load();
  }, []);
  ...
  return (
    <>
      ...
      <main className="bg-neutral-50 dark:bg-black text-neutral-900 dark:text-white overflow-x-hidden">
        ...
        {/* ── MURALS & DOMES ── */}
        <section id="murals" className="bg-neutral-100 dark:bg-[#0a0a0a] py-20 md:py-40 overflow-hidden">
          <div className="text-center mb-16 md:mb-32 select-none px-4 md:px-6">
            <motion.h2 initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
              ...
              <span className="block text-4xl sm:text-5xl md:text-8xl font-black tracking-[0.2em] md:tracking-[0.3em] uppercase
                               mb-[-16px] sm:mb-[-20px] md:mb-[-40px] murals-outline-text">
            ...
            <motion.div initial={{ width:0 }} whileInView={{ width:"100px" }}
              ...
              className="h-[1px] mx-auto mt-10 md:mt-12"
              style={{ background:"linear-gradient(to right, transparent, #b8955a, transparent)" }}/>
          </div>

          <div className="overflow-hidden">
            <div className="flex gap-6 md:gap-14 animate-scroll w-max hover:[animation-play-state:paused]"
              style={{ animationDuration:"70s" }}>
              {[...cfg.murals, ...cfg.murals].map((m,i)=>(
                <div key={i}
                  className="relative group flex-shrink-0 cursor-pointer w-[80vw] sm:w-[420px] md:w-[640px]"
        ...
        {/* ── VARIOUS WORKS ── */}
        <section id="various" className="px-4 sm:px-8 md:px-28 py-20 md:py-40 bg-neutral-100 dark:bg-neutral-950">
          <div className="w-full h-[1px] bg-gradient-to-r from-transparent via-[#b8955a]/40 to-transparent mb-12 md:mb-20"/>
        ...
        {/* ── ABOUT ── */}
        <section id="about" className="py-24 md:py-60 px-4 sm:px-8 md:px-32 bg-[#f9f9f9] dark:bg-[#080808] overflow-hidden">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-20 items-center"
               dir={isAR?"rtl":"ltr"}>
            ...
            <div className="lg:col-span-7 space-y-10 md:space-y-16">
              ...
              <motion.div className="flex flex-wrap gap-3 md:gap-6 pt-4 md:pt-8"
                initial={{ opacity:0 }} whileInView={{ opacity:1 }}
                transition={{ duration:1, delay:0.9 }} viewport={{ once:true }}>
                {socials.map(s=>(
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 md:gap-3 px-4 md:px-5 py-2.5 md:py-3
                               border border-neutral-300 dark:border-neutral-700
                               text-[10px] md:text-[11px] tracking-[0.35em] uppercase font-medium
        ...
        {/* ── CONTACT ── */}
        <section id="contact" className="bg-neutral-100 dark:bg-neutral-950 py-20 md:py-40 px-4 md:px-6 flex items-center justify-center">
          <ContactForm />
        </section>
