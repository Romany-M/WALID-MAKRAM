...
function ImageLightbox({ items, index, onClose, onNav }: { items: any[]; index: number; onClose: () => void; onNav:(i:number)=>void }) {
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <div className={`flex justify-between items-center px-8 py-5 flex-shrink-0 border-b
        ...
        <button onClick={onClose}
          className={`flex items-center gap-3 text-sm tracking-[0.4em] uppercase transition-colors duration-200 group
            ${isDark ? "text-white/60 hover:text-white" : "text-neutral-500 hover:text-neutral-900"}`}>
          {t.close[lang]}
          <span className={`w-9 h-9 rounded-full border flex items-center justify-center text-lg text-[#b8955a] transition-all duration-300 group-hover:border-[#b8955a]
      ...
      <div className="flex-1 flex flex-col md:flex-row items-stretch overflow-hidden"
        ...
        <div className="flex-1 flex items-center justify-center relative p-8 md:p-16 overflow-hidden">
          <button className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-20 flex items-center justify-center text-white/15 hover:text-[#b8955a] transition-colors duration-300 text-5xl z-10"
            onClick={() => onNav((index-1+total)%total)}>‹</button>
          <motion.div key={index} initial={{ opacity:0, scale:0.97 }} animate={{ opacity:1, scale:1 }}
          ...
          <button className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-20 flex items-center justify-center text-white/15 hover:text-[#b8955a] transition-colors duration-300 text-5xl z-10"
        ...
        <motion.div key={`info-${index}`}
          ...
          className={`w-full md:w-[500px] shrink-0 flex flex-col justify-center px-12 py-14
                     border-t md:border-t-0 md:border-l
                     ${isDark ? "border-white/10 bg-transparent" : "border-neutral-200 bg-white"}`}>
          <p className="text-[#b8955a] text-[10px] tracking-[0.65em] uppercase mb-6">— {item.year} —</p>
          <h3 className={`font-extralight leading-relaxed mb-10 ${isDark?"text-white":"text-neutral-900"} ${isAR ? "ar-section-title text-2xl" : "text-3xl tracking-[0.15em]"}`}>
          ...
          <div className={`space-y-6 border-t pt-8 ${isDark?"border-white/10":"border-neutral-200"}`}>
            <div>
              ...
              <p className={`font-light leading-relaxed ${isDark?"text-neutral-300":"text-neutral-600"} ${isAR?"ar-card-text text-base":"text-sm italic"}`}>{getF(item.medium,"mediumAR")}</p>
            </div>
            {item.dims&&<div>
            ...
            <div>
              ...
              <p className={`leading-relaxed ${isDark?"text-neutral-400":"text-neutral-500"} ${isAR?"ar-card-text text-base":"text-sm tracking-wider"}`}>{getF(item.location,"locationAR")}</p>
            </div>
          </div>
          <div className="flex gap-3 mt-12">
            <button onClick={()=>onNav((index-1+total)%total)}
              className={`flex-1 border py-4 text-sm tracking-[0.3em] uppercase hover:border-[#b8955a] transition-all duration-300
      ...
      <div className="overflow-hidden bg-neutral-100 dark:bg-neutral-900 relative aspect-[4/5]">
        <img src={src} alt={isAR?titleAR:title}
          className="absolute inset-0 w-full h-full object-cover object-top
                     group-hover:scale-105 transition-all duration-1000 ease-in-out" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#b8955a]/30 via-transparent to-transparent
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-400">
          <div className="w-11 h-11 rounded-full border border-white/60 flex items-center justify-center backdrop-blur-sm bg-black/20">
            <svg viewBox="0 0 24 24" className="w-4 h-4 stroke-white fill-none" strokeWidth="1.6"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>
          </div>
        </div>
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-black/60 translate-y-full group-hover:translate-y-0 transition-transform duration-500"
             dir={isAR?"rtl":"ltr"}>
          <p className="text-white text-[10px] tracking-[0.35em] uppercase font-medium">{isAR?titleAR:title}</p>
          <p className="text-[#f0cc8a] text-[9px] tracking-[0.3em] mt-1">{isAR?locationAR:location}</p>
        </div>
      </div>
      <div className="mt-4 pb-3 border-b border-neutral-200 dark:border-neutral-800 flex justify-between items-baseline gap-3"
           dir={isAR?"rtl":"ltr"}>
        <div>
          <h4 className={`uppercase font-medium leading-relaxed ${isAR?"ar-card-text":"text-[10px] tracking-[0.25em]"}`}>{isAR?titleAR:title}</h4>
          <p className={`italic mt-1 ${isAR?"ar-card-text":"text-[9px] text-neutral-500 tracking-wide"}`}>{isAR?mediumAR:medium}</p>
        </div>
        <div className={`flex-shrink-0 ${isAR?"text-left":"text-right"}`}>
          <span className="text-[9px] tracking-widest text-neutral-400 block">{dims}</span>
          <span className="text-[9px] tracking-widest text-neutral-400 block">{year}</span>
        </div>
      </div>
    </motion.div>
  );
...
function SectionHeader({ label, title }: { label:string; title:string }) {
  return (
    <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
      viewport={{ once:true }} transition={{ duration:1 }} className="text-center mb-20">
      <p className="text-[9px] tracking-[0.55em] text-neutral-400 uppercase mb-6">{label}</p>
      <h2 className="font-light tracking-[0.5em] uppercase italic text-3xl md:text-4xl">{title}</h2>
      <div className="w-8 h-[1px] bg-[#b8955a] mx-auto mt-8" />
    </motion.div>
  );
}
...
function GallerySection({ data }: { data: GalleryConfig["galleryData"] }) {
  ...
  const [active, setActive] = useState<"ancient"|"coptic"|"oil">(() => {
    ...
    if (saved === "coptic" || saved === "oil" || saved === "ancient") {
  ...
  return (
    <>
      ...
      <section id="gallery" className="px-10 md:px-28 py-40">
        <SectionHeader label={t.selected_works[lang]} title={t.gallery_title[lang]} />
        <div className="flex items-center justify-center mb-20" dir={isAR?"rtl":"ltr"}>
          <div className="flex border border-neutral-200 dark:border-neutral-800">
            {galleryTabs.map(tab=>(
              <button key={tab.key}
                ...
                className={`relative px-10 py-3 text-[9px] tracking-[0.4em] uppercase transition-all duration-500
        ...
        <motion.div key={active} initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:0.6 }}
          className="grid grid-cols-2 md:grid-cols-4 gap-6 items-start">
          {items.map((item,i)=><ArtCard key={i} {...item} delay={i*0.07} onClick={()=>setLb(i)}/>)}
        </motion.div>
        <div className="text-center mt-20">
          <Link href={`/gallery/${active}`}
            className="inline-flex items-center gap-4 border border-neutral-300 dark:border-neutral-700
                       px-10 py-3 text-sm tracking-[0.3em] uppercase
  ...
  return (
    <>
      ...
      <main className="bg-neutral-50 dark:bg-black text-neutral-900 dark:text-white overflow-x-hidden">
            ...
            {isAR ? (
              <div className="mb-4 flex flex-col items-center gap-3">
                ...
                <div className="flex items-center gap-4 w-full justify-center">
                  <span className="h-[1px] w-16 bg-gradient-to-r from-transparent to-[#b8955a]"/>
                  <span className="text-[#b8955a] text-[10px] tracking-[0.5em]">✦</span>
                  <span className="h-[1px] w-16 bg-gradient-to-l from-transparent to-[#b8955a]"/>
                </div>
              </div>
            ) : (
        ...
        {/* ── MURALS & DOMES ── */}
        <section id="murals" className="bg-neutral-100 dark:bg-[#0a0a0a] py-40 overflow-hidden">
          <div className="text-center mb-32 select-none px-6">
            <motion.h2 initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
              transition={{ duration:1 }} viewport={{ once:true }} className="relative inline-block">
              <span className="block text-5xl md:text-8xl font-black tracking-[0.3em] uppercase mb-[-20px] md:mb-[-40px] murals-outline-text">
                {isAR?"الجداريات":"Murals"}
              </span>
              <span className="block text-4xl md:text-6xl font-extralight tracking-[0.5em] uppercase text-[#b8955a]"
                style={{ textShadow:"0 0 15px rgba(184,149,90,0.3)" }}>
                {isAR?"والقباب":"& Domes"}
              </span>
            </motion.h2>
            <motion.div initial={{ width:0 }} whileInView={{ width:"100px" }}
          ...
          <div className="overflow-hidden">
            <div className="flex gap-14 animate-scroll w-max hover:[animation-play-state:paused]"
        ...
        {/* ── VARIOUS WORKS ── */}
        <section id="various" className="px-10 md:px-28 py-40 bg-neutral-100 dark:bg-neutral-950">
          <div className="w-full h-[1px] bg-gradient-to-r from-transparent via-[#b8955a]/40 to-transparent mb-20"/>
        ...
        {/* ── ABOUT ── */}
        <section id="about" className="py-60 px-10 md:px-32 bg-[#f9f9f9] dark:bg-[#080808] overflow-hidden">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-20 items-center"
               dir={isAR?"rtl":"ltr"}>
            <motion.div className="lg:col-span-5 relative"
              initial={{ opacity:0, x:isAR?50:-50 }} whileInView={{ opacity:1, x:0 }}
              ...
              <p className="mt-8 text-[10px] tracking-[0.5em] uppercase text-neutral-400 text-center">
            ...
            <div className="lg:col-span-7 space-y-16">
              <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
                ...
                <span className={`text-[#b8955a] uppercase block mb-4 ${isAR?"ar-label":"text-[11px] tracking-[0.6em]"}`}>{t.philosophy[lang]}</span>
                <h2 className={`font-extralight leading-tight ${isAR?"ar-section-title":"text-4xl md:text-5xl tracking-[0.2em]"}`}>
                  {t.beyond[lang]} <br/> {t.visible[lang]}
                </h2>
              </motion.div>
              <motion.div className="space-y-8 text-neutral-600 dark:text-neutral-400 font-light leading-[2.2] text-lg max-w-xl"
                initial={{ opacity:0 }} whileInView={{ opacity:1 }}
                transition={{ duration:1, delay:0.6 }} viewport={{ once:true }}>
                <p className={isAR?"ar-description":""}>{t.about_p1[lang]}</p>
                <p className={`${isAR?"border-r-2 pr-8 ar-quote":"border-l-2 pl-8"} border-[#b8955a] py-2 italic`}>{t.about_quote[lang]}</p>
                <p className={isAR?"ar-description":""}>{t.about_p2[lang]}</p>
              </motion.div>
              <motion.div className="flex flex-wrap gap-6 pt-8"
                initial={{ opacity:0 }} whileInView={{ opacity:1 }}
